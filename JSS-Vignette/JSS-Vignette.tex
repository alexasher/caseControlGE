...
parametric modelling of the genetic variables. The \pkg{caseControlGE}
package implements the methods of \citeauthor{Stalder2017} (2017,
\emph{Biometrika}, \textbf{104}, 801-812) and
\citeauthor{Wang2018unpublished} (2018, unpublished), which exploit the
assumption of gene-environment independence without placing any
...
retrospective method; semiparametric analysis; polygenic analysis}
...
distributed with the \pkg{caseControlGE} package. Fortunately, we can
use the \pkg{caseControlGE} function \code{simulateCC} to generate a
...
status). \code{simulateCC} works by simulating a population using
user-specified parameters, then selecting \code{ncase} cases and
\code{ncontrol} controls as the case-control sample. To accurately
...
\citeauthor{Stalder2017}, and is the reason that \code{spmle} requires
the user to specify a value for \code{pi1}.)
